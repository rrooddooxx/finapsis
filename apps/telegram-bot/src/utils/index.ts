...
export const sendMsg = async ({ message, ctx }: { message: string, ctx: Context }) => {
...
export const sendFileToBackend = async ({ fileUrl, ctx}: { fileUrl: string, ctx: Context }) => {
  ...
  if (!res.ok) {
    ...
    return {
      message: 'Lo siento, ocurrió un error al subir tu archivo 📎. Por favor, inténtalo nuevamente en unos segundos.',
    }
  }

  return {
    message: '✅ ¡Listo! Tu archivo se subió correctamente y comenzaremos el análisis. Te avisaré cuando tenga resultados. 🔍',
  }
}