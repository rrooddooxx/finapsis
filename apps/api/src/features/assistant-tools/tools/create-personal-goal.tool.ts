...
import { createPersonalGoal, calculateGoalProgress } from "../../personal-goals/goals.service";

export const createPersonalGoalTool = tool({
    description: "Create a new personal financial goal when the user expresses intent to set a financial target or objective",
    inputSchema: createPersonalGoalSchema,
    execute: async (params: any, context: any): Promise<CreatePersonalGoalResponse> => {
        try {
            console.log('🎯 CREATE_PERSONAL_GOAL tool called:', params);

            const { userId, goalType, targetAmount, currentAmount, targetDate, description } = params;
