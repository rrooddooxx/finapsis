...
import { updatePersonalGoal, calculateGoalProgress } from "../../personal-goals/goals.service";

export const updatePersonalGoalTool = tool({
    description: "Update an existing personal financial goal's progress, status, or details",
    inputSchema: updatePersonalGoalSchema,
    execute: async (params: any, context: any): Promise<UpdatePersonalGoalResponse> => {
        try {
            console.log('📝 UPDATE_PERSONAL_GOAL tool called:', params);

            const { userId, goalId, currentAmount, status, description, targetAmount, targetDate } = params;
            ...
            if (status === 'completed') {
                ...
                message = `Progreso actualizado. ${progress ? `Progreso actual: ${progress.toFixed(1)}%` : ''}`;
            }
            ...
            return {
                ...
                goal: {
                    ...
                    targetAmount: updatedGoal.targetAmount || null,
                    currentAmount: updatedGoal.currentAmount || '0',
                    targetDate: updatedGoal.targetDate || null,
