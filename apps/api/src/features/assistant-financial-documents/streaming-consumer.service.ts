...
export class StreamingConsumerService {
    ...
    private getPriorityBasedOnSource(source: DocumentUploadJobData['source']): number {
        ...
        switch (source) {
            case 'whatsapp':
                return 20; // Highest priority for WhatsApp uploads
            case 'web':
                return 15;      // Medium priority for web uploads
            case 'api':
                return 10;      // Standard priority for API uploads
            default:
                return 5;          // Lowest priority for unknown sources
        }
    }
